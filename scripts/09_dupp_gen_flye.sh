...
ATH_GFF=/data/users/mwannier/annotation/MAKER/flye/flye.maker.output/flye.all.gff
FLYE=/data/users/mwannier/annotation/MAKER/flye/flye.maker.output
WORKDIR=/data/users/mwannier/annotation/dupp_gen/flye
...
#makeblastdb -in $FLYE/flye.all.maker.proteins.fasta -dbtype prot -title Ath -out $WORKDIR/blast_db/Ath
# Align protein query sequences against the reference database
#blastp -query $FLYE/flye.all.maker.proteins.fasta -db $WORKDIR/blast_db/Ath -evalue 1e-10 -max_target_seqs 5 -outfmt 6 -num_threads $SLURM_CPUS_PER_TASK -out $WORKDIR/Ath.blast
...
#2) Run DupGen_finder
#$DUPGEN/DupGen_finder.pl -i $WORKDIR -t Ath -c Nnu -o $OUTDIR/flye
...
#seqkit grep -f ID.txt $FLYE/flye.all.maker.transcripts.fasta -o C24.wgd.genes.fa
